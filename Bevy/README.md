...
> **AI agents:** read [`AGENTS.md`](./AGENTS.md) first for repo conventions and the full folder map.
...
For an optimized distribution build:
```cmd
...
**Development server** (auto-rebuilds on code changes, served at `http://localhost:8080`):
```cmd
trunk serve documentation/index.html
```

**Production web build** (outputs to `dist/`):
```cmd
trunk build --release documentation/index.html
```

# What does this template give you?

* A Bevy-first ECS source layout under `src/Runtime/Client/`
* **`Components/`** — marker/data components attached to entities
* **`Resources/`** — global state, typed asset handles, and input intent
* **`Systems/`** — one Bevy Plugin per feature file
* **`Misc/`** — menu, app entrypoints, and other files that are not components, resources, or systems
* **`Runtime/Shared/`** and **`Runtime/Server/`** — scaffolding for cross-cutting or headless code
* **`src/Tests/`** — headless in-crate test modules for resources, components, and game state
* **`AGENTS.md`** — copy-paste prompt for AI coding agents
* Asset loading via [`bevy_asset_loader`](https://github.com/NiklasEi/bevy_asset_loader) with typed collections
* Audio via [`bevy_kira_audio`](https://github.com/NiklasEi/bevy_kira_audio) (reliable on Web and Android)
* Input abstraction: raw input → `Actions` resource → all other systems
* Two-state machine: `Loading → Playing`
* small example ["game"](https://niklasei.github.io/bevy_game_template/)
* easy setup for running the web build using [trunk] (`trunk serve documentation/index.html`)
...
# How to use this template?
 ...
 4. Start coding :tada:
    ...
    * Start the web build: `trunk serve documentation/index.html`
...
You should keep the `documentation/credits` directory up to date. The release workflow automatically includes the directory in every build.
...
For Bevy-specific learning, use the [Bevy learning page][bevy-learn], the [Bevy Cheat Book][Bevy Cheat Book], and the [official Bevy Discord server][bevy-discord]. For platform setup details, the Bevy example README covers [Android][android-instructions] and [iOS][ios-instructions].
...
This project is licensed under [CC0 1.0 Universal](LICENSE) except some content of `assets` and the Bevy icons in the `build` directory (see [Credits](documentation/credits/CREDITS.md)). Go crazy and feel free to show me whatever you build with this ([@nikl_me][nikl-twitter] / [@nikl_me@mastodon.online][nikl-mastodon] ).
