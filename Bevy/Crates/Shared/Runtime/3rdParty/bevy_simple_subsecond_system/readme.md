...
This is a minimal vendored copy used by this project. Nonessential upstream files such as examples, CI config, tests, local lockfile, and the proc-macro subcrate were removed to keep the repository small.

Local reason for vendoring: keep the package patched for Bevy `0.18.1` while preserving the normal `bevy_simple_subsecond_system` API used by the game. The macro helper still comes from crates.io as `bevy_simple_subsecond_system_macros = "0.1.9"`.

Upstream license: `MIT OR Apache-2.0`. Upstream source and package metadata:
